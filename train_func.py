...
def load_architectures(name, low_quant_func,high_quant,num_classes):
    ...
    _name = name.lower()
    if _name == "resnet18":
        from architectures.resnet_lp import ResNet18
        net = ResNet18(low_quant_func,high_quant,num_classes)
...
def load_checkpoint(model_dir, low_quant_func, high_quant, num_classes, epoch=None, eval_=False, lp_config=None):
    ...
    if lp_config is not None:
        ckpt_dir = os.path.join(ckpt_dir,lp_config)
    ...
    net = load_architectures(params['arch'], low_quant_func,high_quant, num_classes)
